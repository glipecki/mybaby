...
import {PastSleepsSumSleepPipe} from '../../sleep/past-sleeps/past-sleeps-sum-sleep.pipe';
import {SleepType} from '../../sleep/sleep-type';

@Component({
    selector: 'bb-list-with-day-grouping',
    template: `
        <div *ngFor="let row of csv">
            {{row.date}}    {{row.sleep}}
        </div>
...
export class ListWithDayGroupingComponent<T> implements OnChanges {
    ...
    csv: { date: string; sleep: string }[];

    constructor(private pastSleepsSumSleepPipe: PastSleepsSumSleepPipe) {
    }
    ...
    ngOnChanges(changes: SimpleChanges): void {
        ...
        // this.csv = this.grouped.map(group => ({
        //   date: group.dateString,
        //   sleep: this.pastSleepsSumSleepPipe.sumSleepTime(group)
        // }));

        this.csv = this.grouped
            .map(group => ({
                ...group,
                rows: group.rows.filter((row: any) => row.type === SleepType.night),
            }))
            .map(group => ({
                date: group.dateString,
                sleep: this.pastSleepsSumSleepPipe.sumSleepTime(group as any)
            }));
    }
